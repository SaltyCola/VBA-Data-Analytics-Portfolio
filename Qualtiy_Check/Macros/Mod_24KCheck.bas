Attribute VB_Name = "Mod_24KCheck"
Public Sub QCLiveTrackerCheck()
    ...
    For Each wrkb In Application.Workbooks
        If wrkb.Name = "24k Quality Clinic Live Tracker.xlsm" Then
            Set wrkbQC = wrkb
            srcQC = True
            Exit For
        End If
    Next wrkb
    For Each wrkb In Application.Workbooks
        If Right(wrkb.Name, 32) = "24K & 17K Blade WIP Tracker.xlsm" Then
            Set wrkbWIP = wrkb
            srcWIP = True
            Exit For
        End If
    ...
    cntWIP = 0
    j = 0
    For Each c1 In wrkbWIP.Worksheets("24K tab").Range("5:5")
        'exit for
        If c1.Interior.Color = RGB(192, 0, 0) Then
            Exit For
