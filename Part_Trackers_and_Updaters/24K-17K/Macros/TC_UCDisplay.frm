...
'For Last Date Seen: If no change to last date seen textbox, then apply today's date.
    ...
    Dim bUCNonDate As Boolean 'True: This textbox belongs to a non-date value op row
    ...
    For Each ctrl In Me.frameOperations.Controls
        ...
        If TypeName(ctrl) = "TextBox" Then
            ...
            'value change?
                ...
                'test for non-date row
                bUCNonDate = False
                For i1 = 1 To UBound(arrNonDateOpRows)
                    If arrNonDateOpRows(i1) = cIndex + Me.UCShown.Headers.GroupSize Then
                        bUCNonDate = True
                        Exit For
                    End If
                Next i1
                ...
                'non-date value rows
                If bUCNonDate Then
                    'UCString <> txtbox.Value
                    If Me.UCShown.OperationsList(cIndex).UCString <> CStr(ctrl.Value) Then
                        'boolean
                ...
                'date value rows {subtract year from UCDate with: left( ..., (len(...) - 5) )}
                Else
                    ...
                    If (Me.UCShown.OperationsList(cIndex).UCDate <> CDate(0)) And (CStr(ctrl.Value) <> Left(CStr(Me.UCShown.OperationsList(cIndex).UCDate), (Len(CStr(Me.UCShown.OperationsList(cIndex).UCDate)) - 5))) Then
                        ...
                        'value
                            ...
                            ElseIf Len(CStr(ctrl.Value)) >= 8 And Len(CStr(ctrl.Value)) <= 10 And IsDate(CStr(ctrl.Value)) Then
                                arrUCChanges(2, 1)(cIndex, 1) = CStr(ctrl.Value)
                            'incorrect format, reverse boolean's value to no change
                            Else
...
'Commented out as per Amy's request.                            ArrWIP(ucIndex).LastDateSeen = Date 'alter last date seen
'Commented out as per Amy's request.                            arrUCChanges(3, 1)(4, 1) = CStr(Date) 'alter in changes array to give to notes.valueslist as well
'Commented out as per Amy's request.                            arrUCChanges(3, 1)(4, 2) = True 'alter corresponding changes array boolean to true
        ...
        For i1 = 1 To ArrWIP(ucIndex).NumberOfOps
            'test for non-date row
            bUCNonDate = False
            For i2 = 1 To UBound(arrNonDateOpRows)
                If arrNonDateOpRows(i2) = i1 + Me.UCShown.Headers.GroupSize Then
                    bUCNonDate = True
                    Exit For
                End If
            Next i2
            ...
            If bUCNonDate Then
                'apply value changes
                If arrUCChanges(2, 1)(i1, 2) = True Then
                    ArrWIP(ucIndex).OperationsList(i1).UCString = CStr(arrUCChanges(2, 1)(i1, 1))
                End If
            'date value row
            Else
...
'Commented out as per Amy's request.                arrUCChanges(3, 1)(4, 2) = True 'alter corresponding changes array boolean to true
            ...
            For i2 = 1 To ArrWIP(ucIndex).NumberOfOps
                'test for non-date row
                bUCNonDate = False
                For i3 = 1 To UBound(arrNonDateOpRows)
                    If arrNonDateOpRows(i3) = i2 + Me.UCShown.Headers.GroupSize Then
                        bUCNonDate = True
                        Exit For
                    End If
                Next i3
                ...
                If bUCNonDate Then
                    'apply value changes
                    ArrWIP(ucIndex).OperationsList(i2).UCString = CStr(arrUCChanges(2, 1)(i2, 1))
                'date value row
                Else
                    'apply value changes if it's a date
...
Public Sub InfoUpdate_LastOpCompleted(ByVal ucIndex As Integer)
    ...
    For i = 1 To ArrWIP(ucIndex).NumberOfOps
        'look for first non blank, non dark purple and enabled OpRow
...
Public Sub InfoUpdate_RTO(ByVal ucIndex As Integer) '<==Alter (Entire Sub, Unique)
    ...
    For i = 1 To ArrWIP(ucIndex).NumberOfOps
        ...
        If ArrWIP(ucIndex).OperationsList(i).UCColor = ArrWIP(ucIndex).OperationsList(1).UCColorList.RTO And ArrWIP(ucIndex).OperationsList(i).Enabled Then
            ...
            ArrWIP(ucIndex).Notes.ValuesList(1) = "rto"
...
'Check info in Operations frame for errors, before save.
    ...
    For Each ctrl In Me.frameOperations.Controls
        'test for non-date value row
        bUCNonDate = False
        For i = 1 To UBound(arrNonDateOpRows)
            If TypeName(ctrl) = "TextBox" Then
                If arrNonDateOpRows(i) = CInt(Me.frameOperations.Controls("rOps" & Int(Replace(ctrl.Name, "txtOps", ""))).Caption) Then
                    bUCNonDate = True
                    Exit For
                End If
            End If
        Next i
        'only date value textboxes need validation (ignore non-date value textboxes)
        If TypeName(ctrl) = "TextBox" And Not bUCNonDate Then
            If (ctrl.Value <> "rescan") Then 'ignore "TAI rescan"
                'values validation
...
'Check info in Notes frame for errors, before save.
    ...
    'Last Date Seen
        'set ctrl variable
        Set ctrl = Me.frameNotes.Controls("txtN" & 10)
        'values validation
        For i = 1 To Len(ctrl.Value)
            If InStr(1, strAllowedChars, Mid(ctrl.Value, i, 1)) = 0 Then
                'Error Message
...
'Read data from ArrWIP to populate userform and reset Data Validation Booleans.
    ...
    Dim i As Integer 'iterator
    Dim i1 As Integer 'iterator
    Dim bUCNonDate As Boolean 'True: This row can have a non-date value
    Dim ctrl As Control 'iterator
    ...
    For i = 1 To Me.UCShown.NumberOfOps
        ...
        'fill values (ignore 0-Date values)
            'check for member of non-date rows
            bUCNonDate = False
            For i1 = 1 To UBound(arrNonDateOpRows)
                If arrNonDateOpRows(i1) = (i + Me.UCShown.Headers.GroupSize) Then
                    bUCNonDate = True
                    Exit For
                End If
            Next i1
            'non-date value rows or "TAI rescan"
            If bUCNonDate Or (Me.UCShown.OperationsList(i).UCString = "rescan") Then
                'fill textbox with string value
                Me.frameOperations.Controls("txtOps" & i).Value = Me.UCShown.OperationsList(i).UCString
            'date value rows
            ElseIf Me.UCShown.OperationsList(i).UCDate <> CDate(0) Then
