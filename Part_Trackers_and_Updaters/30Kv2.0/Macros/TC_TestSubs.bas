...
Public Sub Fix_ShippedDates()
    ...
    For Each c In ActiveSheet.Range("CJG8:DST46")
        If Not IsEmpty(c) And IsDate(c.Value) Then
            If CLng(Cells(7, c.Column).Value) - CLng(c.Value) < 0 Then
'                If Right(CStr(c.Value), 1) = "7" Then
'                    'switch 2017 to 2016
'                    c.Value = Left(CStr(c.Value), (Len(CStr(c.Value)) - 1)) & "6"
'                End If
                Application.Goto c
                Set c = c
            End If
