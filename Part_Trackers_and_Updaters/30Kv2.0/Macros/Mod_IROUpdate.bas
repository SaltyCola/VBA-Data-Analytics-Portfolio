...
Sub IROUpdate()
    ...
    Set wrks = wrkb.Worksheets("NEO 5322121")
    
    'grab number of columns
    For Each c In wrks.Range("6:6")
        If c.Column > 2 And Not IsEmpty(c) Then
            'increment counter
            numCols = numCols + 1
            'add SN to array
        ...
        'ignore empty redline cells, but exit on first non red empty cell
        ElseIf c.Column > 2 And IsEmpty(c) And Not c.EntireColumn.Interior.Color = RGB(255, 0, 0) Then
            Exit For
        End If
    Next c
